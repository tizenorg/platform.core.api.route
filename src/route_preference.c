 ...
 * Internal macros
 */
#define ROUTE_PREFERENCE_CHECK_CONDITION(condition,error,msg)	\
	if(condition) {} else	\
	{ LOGE("[%s] %s(0x%08x)", __FUNCTION__, msg, error); return error; };	\

#define ROUTE_PREFERENCE_PRINT_ERROR_CODE_RETURN(code)	\
	LOGE("[%s] %s(0x%08x)", __FUNCTION__, #code, code); return code;	\

#define ROUTE_PREFERENCE_NULL_ARG_CHECK(arg)\
	ROUTE_PREFERENCE_CHECK_CONDITION( (arg != NULL), ROUTE_ERROR_INVALID_PARAMETER, "ROUTE_ERROR_INVALID_PARAMETER")

#define ROUTE_PREFERENCE_NULL_ARG_CHECK_RETURN_FALSE(arg)\
	if(arg != NULL) {} else	\
	{ LOGE("[%s] ROUTE_ERROR_INVALID_PARAMETER", __FUNCTION__); return false; };	\
...
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK(preference);

	route_preference_s *handle = (route_preference_s *) malloc(sizeof(route_preference_s));
	if (handle == NULL) {
		ROUTE_PREFERENCE_PRINT_ERROR_CODE_RETURN(ROUTE_ERROR_OUT_OF_MEMORY);
	}
	...
	if (handle->preference == NULL) {
		ROUTE_PREFERENCE_PRINT_ERROR_CODE_RETURN(ROUTE_ERROR_SERVICE_NOT_AVAILABLE);
	}
...
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK(preference);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(callback);
...
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK(preference);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(top_left);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(bottom_right);

	route_preference_s *handle = (route_preference_s *) preference;
	LocationBoundary *bbox = location_route_pref_get_bounding_box(handle->preference);

	if (bbox == NULL) {
		ROUTE_PREFERENCE_PRINT_ERROR_CODE_RETURN(ROUTE_ERROR_RESULT_NOT_FOUND);
	}
...
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK(preference);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(max_results);
...
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK(preference);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(goal);
...
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK(preference);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(mode);
...
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK_RETURN_FALSE(preference);
...
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK(preference);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(key);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(value);
...
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK(preference);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(address);
...
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK(preference);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(area);
...
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK(preference);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(constraint);
...
bool route_preference_is_area_to_avoid_supported(route_service_h service, location_bounds_type_e type)
{
	...
	case LOCATION_BOUNDS_POLYGON:
		...
		break;
	default:
		LOGE("[%s]Unknown location_bounds_type_e : %d", __FUNCTION__, type);
		break;
	}
...
int route_preference_foreach_available_constraints(route_service_h service, route_preference_available_constraint_cb callback,
						   void *user_data)
{
	ROUTE_PREFERENCE_NULL_ARG_CHECK(service);
	ROUTE_PREFERENCE_NULL_ARG_CHECK(callback);
